		...
		<div class="row-fluid">
			<form class="form-inline" method="post"
				...
				<input class="input-xlarge" placeholder="用户名..." name="sname"
					type="text" value="${param.sname}"> <input
					...
					<tbody>
						<c:forEach items="${pager.list}" var="item">
							<tr>
								...
								<td><a
									href="<%=basePath%>sys/user?cmd=toedit&id=${item.userid}">编辑</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<div class="pagination pagination-right">
					<ul>
						...
						<li><c:if test="${pager.pagectrl.currentindex==1}" var="fp">
								...
								<a
									href="<%=basePath%>sys/user?cmd=list&index=${pager.pagectrl.currentindex-1}">上一页</a>
							</c:if></li>
						...
						<li><c:if
								...
								<a
									href="<%=basePath%>sys/user?cmd=list&index=${pager.pagectrl.currentindex+1}">下一页</a>
							</c:if></li>
					</ul>
				</div>
			</div>
