			...
			<form method="post" action="<%=basePath%>sys/role?cmd=saveright">
				<div class="btn-toolbar">
					<input type="submit" class="btn btn-primary" value="保存 "> <a
						href="<%=basePath%>sys/role?cmd=list" class="btn">取消</a> <input
				...
				<div class="well">
					<c:forEach items="${list}" var="fun">
						<c:if test="${fun.funpid==\"-1\"}">
							<div class="toc">
								<h3>
									<input type="checkbox" onclick="selectparent(this)"
										ma="${fun.funid}" name="ckrr" value="${fun.funid}"
										<c:if test="${fun.rr==\"1\"}">
											checked="checked"
										</c:if> />
									${fun.funname}
								...
								<ul>
									<c:forEach items="${list}" var="child">
										<c:if test="${child.funpid==fun.funid}">
											<li><input type="checkbox" onclick="selectchild(this)"
												parent="${fun.funid}" name="ckrr" value="${child.funid}"
