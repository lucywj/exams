...
<body>
			...
			<ul class="nav navbar-nav navbar-right">
				<c:choose>
					<c:when test="${userid!=null}">
						<li><a> <c:out value="${sessionScope.user.usertruename}" />
						</a></li>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4 class="panel-title">
						<a data-toggle="collapse" data-parent="#accordion" href="#Radio">单选题（共
							<c:out value="${fn:length(subjects)}"></c:out> 题，每题2分）
						</a>
					</h4>
				</div>
						...
						<div class="panel-body">
							<ol>
								<c:forEach items="${subjects}" var="item" varStatus="status">
									<div class="subject" data-i="${status.index}"
										data-answer="false" data-sid="${item.sid}"
										data-key="${item.skey}" data-state="0" data-skey>
										<li>${item.scontent}</li>
										<ol>
											<li><label><input type="radio" value="A"
													name="${item.sid}">${item.sa}</label></li>
											<li><label><input type="radio" value="B"
													name="${item.sid}">${item.sb}</label></li>
											<li><label><input type="radio" value="C"
													name="${item.sid}">${item.sc}</label></li>
											<li><label><input type="radio" value="D"
													name="${item.sid}">${item.sd}</label></li>
										</ol>
									</div>
								</c:forEach>
							</ol>
	...
	<aside class="processor">
		<section class="time" id="time">00时00分00秒</section>
		<section class="timu">
			<c:forEach items="${subjects}" var="item" varStatus="status">
